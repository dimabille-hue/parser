...
Сохраняет результаты последнего успешного анализа: байты всех трёх форматов экспорта, статистику, первые 50 записей для предпросмотра и одноразовый токен ссылки.

**`xlsx_bytes` может быть `None`** если `openpyxl` не установлен.
...
**Возвращает:** функцию `(str) -> Optional[str]` — имя триггера или `None`.
...
- `save_field()` — нормализует и сохраняет текущее значение в `record`. Учитывает `csv_column` для маппинга. Вызывает валидацию.
- `open_field(name, first_token, err_code, with_collector)` — вызывает `save_field()`, открывает новое поле, при необходимости активирует коллектор.
- `log(msg)` — пишет в `debug_log` если он передан.
...
```python
{
    'total':       10,                          # всего записей
    'clean':       8,                           # без ошибок
    'with_errors': 2,                           # с ошибками
    'error_rate':  20.0,                        # % записей с ошибками
    'top_errors':  [('errFIO1', 3), ('errKAD2', 1)],  # топ-10 кодов
    'field_fill':  {'яч': 10, 'ном': 10, 'фио/орг': 9, 'кад': 8},  # заполненность
    'duplicates':  1,                           # записей с кодом errDUP
}
```
...
5. **Таблица предпросмотра** — первые 50 строк с заголовками
...
*Документация соответствует TriggerParse Engine v3.0, файл `engine_web.py`, 2428 строк.*